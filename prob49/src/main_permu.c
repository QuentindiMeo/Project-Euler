...
static int is_permu(char *n1, char *n2, char *n3)
{
    ...
    for (int i = 0; n1[i]; i++) a1[n1[i] - 48] = a1[n1[i] - 48] + 1;
    for (int i = 0; n2[i]; i++) a2[n2[i] - 48] = a2[n2[i] - 48] + 1;
    for (int i = 0; n3[i]; i++) a3[n3[i] - 48] = a3[n3[i] - 48] + 1;
...
static int permu(void)
{
    uint res = 1488, t;

    for (; res < 10000; res++) {
        if (!my_is_prime(res)) continue;
